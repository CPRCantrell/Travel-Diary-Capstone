...
const Notification = () => {
    ...
    async function deleteNote(note_id){
        try{
            let response = await axios.get(baseUrl+'/notification/'+note_id , auth)
            setNotify(response.data)
        }
        catch{
    ...
    return (
        <>
            <button onClick={()=>setNotification(!notification)}>Notification</button>
            <Modal show={notification} setShow={setNotification}>
